...
import type { IMenuBase, IMenuModel, IMenuModelQuery, IRoleModel, IRoleModelQuery } from '~/api/base/index.type'
...
function cheackType(rule: any, value: number, callback: Function) {
  if ([0, 1, 2].includes(value)) {
    ...
    callback(new Error('角色类型必须是 0（目录）、1（角色）或 2（权限）'))
  }
}

const rules = reactive<FormRules<Partial<IMenuBase>>>({
  type: [
    { validator: cheackType, required: true, message: '请选择角色类型', trigger: 'blur' },
...
//       else
//         this.conditions[0].message = '请输入角色名称'
...
<template>
  <CmsCrudTemplate
    generic="TableColumn ,IMenuModelQuery,null" name="角色" identifier="role" :rules="rules"
    :query-rules="queryRules" :table-data="tableData" :template-data="templateData" :crud-controller="15"
    :query-form="queryForm"
  >
    <template #QueryForm>
      <el-form-item prop="name" label="角色名称">
        <el-input v-model="queryForm.name" minlength="4" placeholder="搜索角色名称" clearable />
      </el-form-item>
      <el-form-item prop="status" label="路劲">
        <el-input v-model="queryForm.status" placeholder="搜索路劲" clearable />
      </el-form-item>
