...
onMounted(() => {
  timer = setInterval(() => {
    const updatedData = monthlyData.value.map(item => ({
      ...item,
      baseline: item.baseline + Math.random() * 1000 - 500,
      actual: item.actual + Math.random() * 800 - 400,
      saving: item.saving + Math.random() * 200 - 100,
    }))
    monthlyData.value = updatedData

    const [baselineSeries, actualSeries, savingSeries] = barChartOptions.value.series as any[]
    baselineSeries.data = updatedData.map(item => item.baseline)
    actualSeries.data = updatedData.map(item => item.actual)
    savingSeries.data = updatedData.map(item => item.saving)
  }, 3000) // 每3秒更新一次数据
})
